...
#define TABLE_MODEL_DATA_KEY "lumeneh-table-model"
...
typedef struct {
  ...
  GPtrArray *rows;  /* LumenehTableRow* */
} LumenehTable;

typedef struct {
  gint start_offset;
  gint end_offset;
  const LumenehLanguageHighlight *language;
} CodeBlockRange;
...
typedef struct {
  ...
  LumenehTable *table_model;
  LumenehTableRow *table_current_row;
  ...
  const LumenehLanguageHighlight *current_code_language;
  gboolean has_output;
  guint trailing_newlines;
} RenderCtx;

static void lumeneh_table_row_free(gpointer data) {
  LumenehTableRow *row = (LumenehTableRow *)data;
  if (!row) {
    return;
  }
  if (row->cells) {
...
static void lumeneh_table_free(gpointer data) {
  LumenehTable *table = (LumenehTable *)data;
  if (!table) {
    return;
  }
  if (table->aligns) {
...
static LumenehTable *lumeneh_table_new(guint col_count) {
  LumenehTable *table = g_new0(LumenehTable, 1);
  table->col_count = col_count;
  table->aligns = g_array_sized_new(FALSE, FALSE, sizeof(MD_ALIGN), col_count);
  table->rows = g_ptr_array_new_with_free_func(lumeneh_table_row_free);
...
static void apply_code_highlighting_for_block(GtkTextBuffer *buffer,
  ...
  lumeneh_code_scan_state_reset(&state);
  ctx.buffer = buffer;
  ...
  while (TRUE) {
    ...
    lumeneh_code_scan_line(range->language, line, &state, on_code_scan_token, &ctx);
...
gchar *markdown_anchor_mark_name(const gchar *fragment) {
  ...
  gchar *name = g_strdup_printf("%s%s", LUMENEH_ANCHOR_MARK_PREFIX, slug);
  g_free(slug);
  return name;
}
...
static void create_heading_anchor(RenderCtx *ctx) {
  ...
  mark_name = g_strdup_printf("%s%s", LUMENEH_ANCHOR_MARK_PREFIX, slug);
...
static void table_search_index_free(gpointer data) {
  LumenehTableSearchIndex *index = (LumenehTableSearchIndex *)data;
  if (!index) {
    return;
  }
  if (index->cells) {
...
static void table_emit_hidden_search_text(RenderCtx *ctx, LumenehTable *table,
                                          GtkTextChildAnchor *anchor) {
  LumenehTableSearchIndex *index;
  ...
  index = g_new0(LumenehTableSearchIndex, 1);
  index->cells = g_array_new(FALSE, FALSE, sizeof(LumenehTableSearchCellRange));
  index->start_offset = gtk_text_iter_get_offset(&ctx->iter);

  for (guint r = 0; r < table->rows->len; r++) {
    LumenehTableRow *row = g_ptr_array_index(table->rows, r);
    if (!row) {
      continue;
    }
    ...
    for (guint c = 0; c < table->col_count; c++) {
      ...
      if (cell_end > cell_start) {
        LumenehTableSearchCellRange cell_range = {(gint)r, (gint)c, cell_start,
                                                 cell_end};
  ...
  if (index->end_offset > index->start_offset) {
    apply_tag_by_name_offsets(ctx->buffer, TAG_INVISIBLE, index->start_offset,
                              index->end_offset);
    g_object_set_data_full(G_OBJECT(anchor), LUMENEH_TABLE_SEARCH_INDEX_DATA, index,
...
static void table_emit_anchor(RenderCtx *ctx) {
  ...
  if (ctx->table_model->rows->len == 0 || ctx->table_model->col_count == 0) {
    lumeneh_table_free(ctx->table_model);
  ...
  g_object_set_data(G_OBJECT(anchor), LUMENEH_TABLE_ANCHOR_DATA, GINT_TO_POINTER(1));
...
static void image_emit_anchor(RenderCtx *ctx) {
  ...
  anchor = gtk_text_buffer_create_child_anchor(ctx->buffer, &ctx->iter);
  note_non_newline_output(ctx);
  g_object_set_data(G_OBJECT(anchor), LUMENEH_IMAGE_ANCHOR_DATA, GINT_TO_POINTER(1));
  g_object_set_data_full(G_OBJECT(anchor), LUMENEH_IMAGE_SRC_DATA,
  ...
  g_object_set_data_full(G_OBJECT(anchor), LUMENEH_IMAGE_ALT_DATA, alt, g_free);
...
static int on_enter_block(MD_BLOCKTYPE type, void *detail, void *userdata) {
  ...
  case MD_BLOCK_CODE:
    ctx->current_code_start_offset = gtk_text_iter_get_offset(&ctx->iter);
    {
      ...
      ctx->current_code_language = lumeneh_code_lookup_language(language);
      g_free(language);
    }
    ensure_newlines(ctx, 2);
  ...
  case MD_BLOCK_TABLE: {
    ...
    ctx->table_model = lumeneh_table_new(tbl ? tbl->col_count : 0);
    ctx->table_current_row = NULL;
...
static int on_enter_span(MD_SPANTYPE type, void *detail, void *userdata) {
  ...
  case MD_SPAN_A: {
    ...
    g_object_set_data_full(G_OBJECT(url_tag), LUMENEH_LINK_URL_DATA, href, g_free);
...
void markdown_init_tags(GtkTextBuffer *buffer) {
  ...
  gtk_text_buffer_create_tag(buffer, LUMENEH_TAG_CODE_KW_A, "family", "Monospace",
                             "foreground", config->h1_color, "weight",
                             PANGO_WEIGHT_BOLD, NULL);
  gtk_text_buffer_create_tag(buffer, LUMENEH_TAG_CODE_KW_B, "family", "Monospace",
                             "foreground", config->h2_color, "weight",
                             PANGO_WEIGHT_BOLD, NULL);
  gtk_text_buffer_create_tag(buffer, LUMENEH_TAG_CODE_KW_C, "family", "Monospace",
                             "foreground", config->h3_color, "weight",
                             PANGO_WEIGHT_BOLD, NULL);
  gtk_text_buffer_create_tag(buffer, LUMENEH_TAG_CODE_LITERAL, "family",
...
void markdown_update_accent_tags(GtkTextBuffer *buffer) {
  ...
  tag = gtk_text_tag_table_lookup(table, LUMENEH_TAG_CODE_KW_A);
  if (tag) {
    g_object_set(tag, "foreground", config->h1_color, NULL);
  }
  tag = gtk_text_tag_table_lookup(table, LUMENEH_TAG_CODE_KW_B);
  if (tag) {
    g_object_set(tag, "foreground", config->h2_color, NULL);
  }
  tag = gtk_text_tag_table_lookup(table, LUMENEH_TAG_CODE_KW_C);
  if (tag) {
    g_object_set(tag, "foreground", config->h3_color, NULL);
  }
  tag = gtk_text_tag_table_lookup(table, LUMENEH_TAG_CODE_LITERAL);
  if (tag) {
    g_object_set(tag, "foreground", config->h3_color, NULL);
  }
...
GtkWidget *markdown_create_table_widget(GtkTextChildAnchor *anchor) {
  ...
  table = (LumenehTable *)g_object_get_data(G_OBJECT(anchor), TABLE_MODEL_DATA_KEY);
  ...
  for (guint r = 0; r < table->rows->len; r++) {
    ...
    for (guint c = 0; c < table->col_count; c++) {
      ...
      g_object_set_data(G_OBJECT(cell), LUMENEH_TABLE_CELL_ROW_DATA,
                        GINT_TO_POINTER((gint)r));
      g_object_set_data(G_OBJECT(cell), LUMENEH_TABLE_CELL_COL_DATA,
